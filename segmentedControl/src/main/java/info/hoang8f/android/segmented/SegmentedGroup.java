...
public class SegmentedGroup extends RadioGroup {
    ...
    public void setBackgroundUncheckedRes(int drawableRes){
        mLayoutSelector.UNSELECTED_LAYOUT=drawableRes;
        updateBackground();
    }
    ...
    private Object mCheckedTag;
    public void check(Object tag) {
        // don't even bother
        if (tag != null && (tag.equals(mCheckedTag))) {
            return;
        }
        check(tag==null?-1:1);

        if (mCheckedTag != null) {
            setCheckedStateForView(mCheckedTag, false);
        }

        if (tag != null) {
            setCheckedStateForView(tag, true);
        }
    ...
    private void setCheckedTag(Object tag) {
        mCheckedTag=tag;
        //listener
    }
     ...
     *
     * @param viewPager
     * @param tabLayout R.layout.filter_radiobutton
     */
    public void setupWithViewPager(ViewPager viewPager, int tabLayout){
        LayoutInflater inflater = LayoutInflater.from(getContext());
        PagerAdapter adapter = viewPager.getAdapter();
        ViewPagerOnSegmentedGroupSelectedListener clickedListener = new ViewPagerOnSegmentedGroupSelectedListener(viewPager);
        if(adapter!=null) {
          //  removeAllViews();
            for(int i=0;i<adapter.getCount();i++) {
                RadioButton radioButton = (RadioButton) inflater.inflate(tabLayout, this, false);
                radioButton.setText(adapter.getPageTitle(i));
                radioButton.setTag((Integer) i);
                radioButton.setOnClickListener(clickedListener);
                addView(radioButton);
            }
            viewPager.addOnPageChangeListener(new SegmentedGroupOnPageChangeListener(this));
            if(mCheckedTag==null || !mCheckedTag.equals(viewPager.getCurrentItem())){
                check((Object)viewPager.getCurrentItem());
            }
        }
        updateBackground();
    }
    public void updateTabTitles(ViewPager pager){
        if(pager.getAdapter()!=null && getChildCount()==pager.getAdapter().getCount()){
            for(int i=0;i<pager.getAdapter().getCount();i++){
                RadioButton button = (RadioButton) findViewWithTag(i);
                button.setText(pager.getAdapter().getPageTitle(i));
            }
        }
    }

    public static class ViewPagerOnSegmentedGroupSelectedListener implements View.OnClickListener{

        private final ViewPager mViewPager;

        public ViewPagerOnSegmentedGroupSelectedListener(ViewPager mViewPager) {
    ...
    private void setCheckedStateForView(Object viewTag, boolean checked) {
        View checkedView = findViewWithTag(viewTag);
    ...
    static class SegmentedGroupOnPageChangeListener implements ViewPager.OnPageChangeListener{
        private final WeakReference<SegmentedGroup> segmentedGroupRef;
        private int mPreviousScrollState;
        private int mScrollState;
        public SegmentedGroupOnPageChangeListener(SegmentedGroup tabLayout) {
            this.segmentedGroupRef = new WeakReference<>(tabLayout);
        }
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            SegmentedGroup tabLayout = (SegmentedGroup)this.segmentedGroupRef.get();
            if(tabLayout != null) {
        ...
        public void onPageSelected(int position) {
            SegmentedGroup segmentedGroup = (SegmentedGroup)this.segmentedGroupRef.get();
            if(segmentedGroup != null) {
                segmentedGroup.check((Object) position);
            }
