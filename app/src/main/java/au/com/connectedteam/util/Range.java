...
public class Range<T extends Comparable<? super T>> implements Serializable{
    ...
    public static <T extends Comparable<? super T>> boolean anyContains(Collection<Range<T>> rangeList, T value){
    ...
    public static <T extends Comparable<? super T>> boolean anyContainsBelowUpperLimits(Collection<Range<T>> rangeList, T value){
    	for(Range<T> range : rangeList){
    		if(range.contains(value) && (value.compareTo(range.getUpper()) < 0)) return true;
    	}
    	return false;
    }
    ...
    /**
     * Checks if the {@code value} is within the bounds of this range, not counting the upper endpoint.
     *
     * <p>A value is considered to be within this range if it's {@code >=}
     * the lower endpoint <i>and</i> {@code <} the upper endpoint (using the {@link Comparable}
     * interface.)</p>
     ...
     * @throws NullPointerException if {@code value} was {@code null}
     */
    public boolean containsBelowUpperLimit(T value) {
    ...
    public int hashCode() {
       // return HashCodeHelpers.hashCode(mLower, mUpper);
    	 int h = (mLower == null) ? 0 : mLower.hashCode();
         int x = (mUpper == null) ? 0 : mUpper.hashCode();
