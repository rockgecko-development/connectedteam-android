...
import au.com.connectedteam.appsapi.generated.ResponseStatus;
...
public class IonHelper {
    ...
    public static final boolean LOG_REQUEST = ConnectedApp.DEBUG;
    public static final boolean LOG_RESPONSE = ConnectedApp.DEBUG;
    ...
    public IonHelper(IonHelperCallbacks configureLoaderCallback){
        ...
        if(sIon==null){
            sIon = Ion.getDefault(ConnectedApp.getContextStatic());
            ion=sIon;
            initDefaults();
        }
        else {
    ...
    private void initDefaults(){
        //ssToIon=new SSToIon(ConnectedApp.getWebserviceUrl());
        ...
        config.setLogging("ION", ConnectedApp.DEBUG ? Log.VERBOSE : 0);
    }
    ...
    public <TResponse> HelperRequest<TResponse> doGet(LoadBuilder<Builders.Any.B> loadBuilder, Class<TResponse> responseClass, String urlPath){
        ...
        String url = ConnectedApp.getWebserviceUrl()+urlPath;
    ...
    public <TResponse> HelperRequest<TResponse> doGet(LoadBuilder<Builders.Any.B> loadBuilder, IReturn<TResponse> request){
        String url = new SSToIon(ConnectedApp.getWebserviceUrl()).createUrl(request);
        Builders.Any.B b2 = loadBuilder.load(url);
        addDefaultHeaders(b2);
    ...
    public <TResponse> HelperRequest<TResponse> doPost(LoadBuilder<Builders.Any.B> loadBuilder, IReturn<TResponse> request){
        String url = new SSToIon(ConnectedApp.getWebserviceUrl()).createUrlNoParams(request);
        Builders.Any.B b2 = loadBuilder.load(url);
        addDefaultHeaders(b2);
        if(LOG_REQUEST){
    ...
    private String getDefaultUserAgentString() {
        ...
        return String.format("%s/%s (%d);android/%s (%d);%s",
                ConnectedApp.getContextStatic().getPackageName(),
